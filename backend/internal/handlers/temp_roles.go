...
func NewTempRoleHandler(db *gorm.DB, ws *WebSocketHandler) *TempRoleHandler {
    return &TempRoleHandler{db: db, ws: ws}
}
...
func (h *TempRoleHandler) GrantTempRole(c fiber.Ctx) error {
    ...
    h.ws.BroadcastToUser(tempRole.UserID.String(), []byte(`{"type":"role_granted","role":"`+string(tempRole.RoleType)+`","target_id":"`+tempRole.TargetID.String()+`"}`))
